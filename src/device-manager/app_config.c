/**
 * @file
 * @brief Wind River IoT service configuration file reader
 *
...
#include "iot_build.h"                 /* for build information */

#include "app_json.h"  /* for json support */
...
struct app_config
{
	app_json_t json;          /**< @brief open configuration file */
};
...
struct app_config *app_config_open( iot_t *iot_lib, const char *file_path )
{
	...
	if ( file_found != IOT_FALSE )
	{
		...
		if ( result )
		{
			...
			os_memzero( result,
				sizeof( struct app_config ) );
			status = app_json_parse_file( &result->json, config_file, 0 );
			if ( status != IOT_STATUS_SUCCESS )
			{
				IOT_LOG( iot_lib, IOT_LOG_ERROR,
					"Error loading configuration file %s; "
					"app_json_parse_file failed : %d",
					config_file, status );
				os_free( (void *)result );
				result = NULL;
			}
	...
	if ( config && field && value )
	{
		...
		if ( group )
		{
			...
			if ( result == IOT_STATUS_SUCCESS )
				result = app_json_object_get( &json_value, &json_group, field );
		}
		else
			result = app_json_object_get( &json_value, &config->json, field );
		if ( result == IOT_STATUS_SUCCESS )
			result = app_json_get_bool_value( value, &json_value );
	}
	return result;
}

iot_status_t app_config_read_string( const struct app_config *config,
	const char *group, const char *field, char *value, size_t max )
{
	iot_status_t result = IOT_STATUS_BAD_PARAMETER;
	if ( config && field && value )
	{
		...
		if ( group )
		{
			...
			result = app_json_object_get( &json_value, &config->json, field );
		if ( result == IOT_STATUS_SUCCESS )
			result = app_json_get_string_value( value, max, &json_value );
	}
	return result;
}
...
iot_status_t app_config_read_uint( const struct app_config *config,
	const char *group, const char *field, unsigned int *value )
{
	iot_status_t result = IOT_STATUS_BAD_PARAMETER;
	if ( config && field && value )
	{
		...
		if ( group )
		{
			...
			result = app_json_object_get( &json_value, &config->json, field );
		if ( result == IOT_STATUS_SUCCESS )
			result = app_json_get_ulong_value( (unsigned long *)value,
				&json_value );
	}
	return result;
}
