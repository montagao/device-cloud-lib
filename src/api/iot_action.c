 ...
 */
static IOT_SECTION iot_status_t iot_action_register(
	iot_action_t *action,
	...
	if ( action && request )
	{
		...
		if ( action->lib )
		{
				...
				param_bad_type_name == NULL; ++i )
			{
				...
				/* check registered parameter vs. request parameter */
				if ( ( reg_param->type & IOT_PARAMETER_IN_REQUIRED )
					...
					/* unable to convert parameter to correct type */
					param_bad_type_name = reg_param->name;
			}

			/* request contains an unknown parameter */
			for ( i = 0u; i < request->parameter_count; ++i )
			{
				/* this value is set by:
				   iot_action_request_parameter_set. If not
				   updated above to an IN parameter, know we
				   didn't get an input value for this parameter.
				   At this point we should have no parameters in
				   the request that are OUT only parameters.
				 */
	...
	iot_type_t type, ... )
{
	...
	if ( action && name )
	{
		...
		va_end( args );
		if ( result == IOT_STATUS_SUCCESS )
			result = iot_action_option_set_data(
				action, name, &data );
	}
	return result;
}
	...
	const struct iot_data *data )
{
	...
	if ( action && data )
	{
		...
		for ( i = 0u;
			opt == NULL && i < action->option_count; ++i )
		{
		...
		if ( !opt && action->option_count < IOT_OPTION_MAX )
		{
#ifndef IOT_STACK_ONLY
...
#endif /* ifndef IOT_STACK_ONLY */
			{
				opt = &action->option[action->option_count];
		...
		if ( opt )
		{
				...
				*opt->name == '\0' )
			{
				...
				if ( name_len > IOT_NAME_MAX_LEN )
					name_len = IOT_NAME_MAX_LEN;
#ifndef IOT_STACK_ONLY
				opt->name = os_malloc( sizeof( char ) * (name_len + 1u) );
				if ( !opt->name )
				{
					result = IOT_STATUS_NO_MEMORY;
					--action->option_count;
			...
			if ( result != IOT_STATUS_NO_MEMORY )
			{
				if ( opt->data.heap_storage )
...
#ifndef IOT_STACK_ONLY
		if ( result != IOT_STATUS_SUCCESS && data->heap_storage )
			os_free( data->heap_storage );
#endif /* ifndef IOT_STACK_ONLY */
	}
	return result;
}
	...
	const void *ptr )
{
	...
	return iot_action_option_set( action, name, IOT_TYPE_RAW, &data );
}
				...
				if ( result == IOT_STATUS_SUCCESS )
				{
...
#ifndef IOT_STACK_ONLY
					{
						...
						if ( !p )
							IOT_LOG( action->lib,
								IOT_LOG_ERROR,
								"No memory available to store parameter: %s",
								name );
					}
#else
					p = &action->parameter[
	...
	iot_millisecond_t max_time_out )
{
	...
	{
#ifdef IOT_STACK_ONLY
		action->command = action->_command;
		action->command[0] = '\0';
#else /* ifdef IOT_STACK_ONLY */
		os_free_null( (void**)&action->command );
#endif /* else IOT_STACK_ONLY */
	...
	iot_type_t type, ... )
{
	...
	if ( request && name )
	{
		...
		va_end( args );
		if ( result == IOT_STATUS_SUCCESS )
			result = iot_action_request_option_set_data(
				request, name, &data );
	}
	return result;
}
	...
	const struct iot_data *data )
{
	...
	if ( request && data )
	{
		...
		/* see if this is an option update */
		for ( i = 0u; opt == NULL && i < request->option_count; ++i )
		{
			if ( os_strcmp( request->option[i].name, name ) == 0 )
				opt = &request->option[i];
		}

		/* adding a new option */
		result = IOT_STATUS_FULL;
		if ( !opt && request->option_count < IOT_OPTION_MAX )
		{
#ifndef IOT_STACK_ONLY
...
#endif /* ifndef IOT_STACK_ONLY */
			{
				opt = &request->option[request->option_count];
				if ( opt )
				{
					os_memzero( opt,
						sizeof( struct iot_option ) );
	...
	const void *ptr )
{
	...
	data.value.raw.length = length;
	data.has_value = IOT_TRUE;
	return iot_action_request_option_set( request, name,
		IOT_TYPE_RAW, &data );
}
	...
	size_t var_data_size )
{
	...
	if ( dest && request && dest != request )
	{
		...
		dest->parameter_count = 0u;
		for ( i = 0u; result == IOT_STATUS_SUCCESS &&
			dest->parameter && i < request->parameter_count; ++i )
		{
	...
	iot_type_t type, ... )
{
	...
	va_list args;
	va_start( args, type );
	result = iot_action_request_parameter_set_args(
		request, name, type, args );
	va_end( args );
	return result;
}
	...
	va_list args )
{
	...
	if ( request && name )
	{
		...
		if ( os_strpbrk( name,
			IOT_PARAMETER_NAME_BAD_CHARACTERS ) == NULL )
		{
			iot_bool_t add_parameter = IOT_FALSE;
			struct iot_action_parameter *p = NULL;
			size_t i;
			...
			/* if parameter not found, then add it */
	...
	iot_millisecond_t duration )
{
	...
	{
		if ( duration == 0u )
			action->flags = action->flags | IOT_ACTION_NO_TIME_LIMIT;
		else
			action->flags = action->flags & ~IOT_ACTION_NO_TIME_LIMIT;
