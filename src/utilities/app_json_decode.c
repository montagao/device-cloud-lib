...
iot_status_t app_json_decode_array_at(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item,
	size_t index,
...
const app_json_array_iterator_t *app_json_decode_array_iterator(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item )
{
	const app_json_array_iterator_t *iter = NULL;
	if ( decoder && item )
	{
#if defined( APP_JSON_JANSSON )
		if ( json_array_size( (const json_t*)item ) > 0u )
			iter = (struct app_json_array_iterator *)1u;
#elif defined( APP_JSON_JSONC )
		struct json_object *j;
		os_memcpy( &j, &item, sizeof( struct json_object * ) );
...
iot_status_t app_json_decode_array_iterator_value(
...
const app_json_array_iterator_t *app_json_decode_array_iterator_next(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item,
	const app_json_array_iterator_t *iter )
{
	const app_json_array_iterator_t *result = NULL;
	if ( decoder && item && iter )
	{
		...
		{
			++i;
			result = (app_json_array_iterator_t*)i;
		}
#elif defined( APP_JSON_JSONC )
		size_t i = (size_t)(iter);
		struct json_object *j;
...
size_t app_json_decode_array_size(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item )
{
	size_t result = 0u;
...
iot_status_t app_json_decode_bool(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item,
	iot_bool_t *value )
{
...
app_json_decoder_t *app_json_decode_initialize(
	void *buf,
	size_t len,
	unsigned int flags )
{
	struct app_json_decoder *decoder = NULL;
#if defined( APP_JSON_JANSSON )
	...
	{
		additional_size = 0u;
		len = sizeof( struct app_json_decoder );
	...
	{
#if defined( APP_JSON_JANSSON )
		decoder = (struct app_json_decoder *)buf;
		decoder->flags = flags;
		decoder->j_root = NULL;
#elif defined( APP_JSON_JSONC )
		decoder = (struct app_json_decoder *)buf;
		...
		{
			unsigned int i;
			decoder = (struct app_json_decoder *)(void*)buf;
			decoder->objs = 0u;
			...
			{
				jsmntok_t *tok;
				tok = (jsmntok_t *)(void*)( (char*)buf + sizeof(struct app_json_decoder) );
				decoder->tokens = tok;
...
const app_json_item_t *app_json_decode_object_find(
	const app_json_decoder_t *decoder,
	const app_json_item_t *object,
	const char *key )
{
	const app_json_item_t *result = NULL;
#if defined( APP_JSON_JANSSON )
...
#else /* defined( APP_JSON_JSMN ) */
	result = app_json_decode_object_find_len( decoder, object, key, 0u );
#endif /* defined( APP_JSON_JSMN ) */
	return result;
}

const app_json_item_t *app_json_decode_object_find_len(
	const app_json_decoder_t *decoder,
	const app_json_item_t *object,
	const char *key,
	size_t key_len )
{
	...
	if ( decoder && object && key )
	{
		...
		{
			char *buf = NULL;
			buf = app_json_realloc( NULL, key_len + 1u );
			if ( buf )
			{
...
const app_json_object_iterator_t *app_json_decode_object_iterator(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item )
{
	const app_json_object_iterator_t *iter = NULL;
	if ( decoder && item )
	{
#if defined( APP_JSON_JANSSON )
		json_t *j_obj;
...
iot_status_t app_json_decode_object_iterator_key(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item,
...
const app_json_object_iterator_t *app_json_decode_object_iterator_next(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item,
	const app_json_object_iterator_t *iter )
{
	const app_json_object_iterator_t *result = NULL;
	if ( decoder && item && iter )
	{
#if defined( APP_JSON_JANSSON )
		json_t *j_item;
		...
		if ( idx >= decoder->objs ||
			cur->start >= obj_end_pos )
			result = NULL;
#endif /* endif( APP_JSON_JSMN ) */
	}
	return result;
}

iot_status_t app_json_decode_object_iterator_value(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item,
	const app_json_object_iterator_t *iter,
	const app_json_item_t **out )
...
size_t app_json_decode_object_size(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item )
{
	size_t result = 0u;
...
iot_status_t app_json_decode_parse(
	...
	if ( decoder && root && js && len > 0u )
	{
		...
		if ( decoder->flags & APP_JSON_FLAG_DYNAMIC )
		{
			...
			if ( !decoder->tokens )
			{
				decoder->size = 1u;
				decoder->tokens =
					(jsmntok_t*)app_json_realloc( NULL,
...
iot_status_t app_json_decode_string(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item,
	const char **value,
	size_t *value_len )
...
void app_json_decode_terminate(
	app_json_decoder_t *decoder )
{
	if ( decoder )
	{
...
app_json_type_t app_json_decode_type(
	const app_json_decoder_t *decoder,
	const app_json_item_t *item )
{
