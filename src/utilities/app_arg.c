...
os_status_t app_get_device_uuid( const char *filename, char *buf, size_t len )
{
	...
	if ( filename && buf )
	{
		os_file_t fd = os_file_open( filename, OS_READ );
		size_t bytes;
		...
		if ( fd  != OS_FILE_INVALID )
		{
			...
			bytes = os_file_read ( buf, sizeof(char), len, fd );
			os_file_close ( fd );
			if( bytes == len )
				buf[bytes - 1u] = '\0';
		...
		else
		{
			...
			if ( len >= 37u )
			{
				...
				fd = os_file_open( filename, OS_WRITE|OS_CREATE );
				if ( fd  != OS_FILE_INVALID )
				{
					bytes = os_file_write( buf, sizeof( char ),
						os_strlen(buf), fd );
					os_file_close ( fd );
