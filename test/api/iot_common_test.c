...
static void test_iot_common_arg_get_bool_from_string_false( void **state )
{
	...
	memcpy( obj.heap_storage, (void *)text, sizeof( text ) );
	obj.has_value = IOT_TRUE;
	result = iot_common_arg_get_wrapper( &obj, IOT_TRUE, IOT_TYPE_BOOL, &data );
	assert_int_equal( result, IOT_STATUS_SUCCESS );
	assert_int_equal( data, IOT_FALSE );
...
static void test_iot_common_arg_get_location_from_location( void **state )
{
	...
	obj.heap_storage = test_malloc( sizeof( iot_location_t ) );
	obj.value.location = (iot_location_t *)obj.heap_storage;
	obj.value.location->accuracy = accuracy;
	obj.value.location->altitude = altitude;
	obj.value.location->altitude_accuracy = altitude_accuracy;
	obj.value.location->heading = heading;
	obj.value.location->latitude = latitude;
	obj.value.location->longitude = longitude;
	obj.value.location->source = source;
	obj.value.location->speed = speed;
	obj.value.location->tag = test_malloc( IOT_NAME_MAX_LEN );
	strncpy( obj.value.location->tag, tag, IOT_NAME_MAX_LEN );
	obj.has_value = IOT_TRUE;
...
static void test_iot_common_arg_set_float32( void **state )
{
	...
	memset( &obj, 0u, sizeof( struct iot_data ) );
	result = iot_common_arg_set_wrapper( &obj, IOT_FALSE, IOT_TYPE_FLOAT32, data );
	assert_int_equal( result, IOT_STATUS_SUCCESS );
	assert_int_equal( obj.has_value, IOT_TRUE );
...
static void test_iot_common_arg_set_location( void **state )
{
	...
	/*will_return( __wrap_os_heap_realloc, 1 );*/
...
static void test_iot_common_arg_set_raw_heap( void **state )
{
	...
	assert_ptr_not_equal( obj.value.raw.ptr, (void *)data );
...
static void test_iot_common_arg_set_raw_no_heap( void **state )
{
	...
	assert_int_equal( obj.type, IOT_TYPE_RAW );
	assert_string_equal( (const char *)obj.value.raw.ptr, "this is some text" );
	assert_ptr_equal( obj.value.raw.ptr, (void *)data );
	assert_int_equal( obj.value.raw.length, sizeof( data ) );
}
...
static void test_iot_common_data_copy_location( void **state )
{
	...
	from.has_value = IOT_TRUE;
	from.heap_storage = test_malloc( sizeof( iot_location_t ) );
	from.value.location = (iot_location_t *)from.heap_storage;
	from.value.location->accuracy = accuracy;
	from.value.location->altitude = altitude;
	from.value.location->altitude_accuracy = altitude_accuracy;
	from.value.location->heading = heading;
	from.value.location->latitude = latitude;
	from.value.location->longitude = longitude;
	from.value.location->source = source;
	from.value.location->speed = speed;
	from.value.location->tag = test_malloc( IOT_NAME_MAX_LEN );
	strncpy( from.value.location->tag, tag, IOT_NAME_MAX_LEN );
	/* will_return( __wrap_os_heap_malloc, 1 ); */
	result = iot_common_data_copy( &to, &from, IOT_TRUE );
	assert_int_equal( result, IOT_STATUS_SUCCESS );
...
static void test_iot_common_data_copy_location_no_memory( void **state )
{
	...
	from.value.location->speed = speed;
	from.value.location->tag = test_malloc( IOT_NAME_MAX_LEN );
	assert_non_null( from.value.location->tag );
	strncpy( from.value.location->tag, tag, IOT_NAME_MAX_LEN );
	will_return( __wrap_os_heap_malloc, 0 );
	result = iot_common_data_copy( &to, &from, IOT_TRUE );
